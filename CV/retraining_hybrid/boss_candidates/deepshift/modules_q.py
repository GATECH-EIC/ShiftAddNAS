...
class Conv2dShiftQ(_ConvNdShiftQ):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 ...
                 rounding='deterministic', weight_bits=5, shift_or_full='shift', threshold=None, quant_bits=16):
    ...
    def forward(self, input):
        if self.shift_or_full == 'shift':
            ...
            weight_q = ste.round_power_of_2(self.weight, self.rounding)
            # print(weight_q)
            input_fixed_point = ste.round_fixed_point(input, self.quant_bits)
            if self.bias is not None:
                bias_fixed_point = ste.round_fixed_point(self.bias, self.quant_bits)
            else:
                bias_fixed_point = None
