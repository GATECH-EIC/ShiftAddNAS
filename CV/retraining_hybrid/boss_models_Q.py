...
from boss_candidates.resnet_op import ResConv, ResAdd, ResShift_Q
    ...
    for stage_idx, (planes, block_encoding, db) in enumerate(zip(channels, encoding, drop_blocks(drop_block_rate))):
        ...
        if num_blocks == 0:
            ...
            for block_idx in range(num_blocks):
                ...
                elif block_encoding[block_idx] == 1:
                ...
                elif block_encoding[block_idx] == 2:
                    blocks.append(ResAdd(
                        inplanes, planes, stride, first_dilation=prev_dilation,
                        drop_path=DropPath(block_dpr) if block_dpr > 0. else None, avg_down=avg_down,
                        act_layer=act_layer, attn_layer=attn_layer))
                elif block_encoding[block_idx] == 3:
                    blocks.append(ResShift_Q(
                        inplanes, planes, stride, first_dilation=prev_dilation,
                        drop_path=DropPath(block_dpr) if block_dpr > 0. else None, avg_down=avg_down,
                        act_layer=act_layer, attn_layer=attn_layer))
                else:
...
class BossNet(nn.Module):
    ...
    def __init__(self, encoding=None, num_classes=1000, in_chans=3,
        ...
        self.global_pool_choice = global_pool

        # print('Origin: ', self.num_features, self.num_classes, self.global_pool_choice)


        self.global_pool, self.fc = create_classifier(self.num_features, self.num_classes,
                                                      pool_type=self.global_pool_choice)
...
class DistilledBossNet(BossNet):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        # print('Distilled: ', self.num_features, self.num_classes, self.global_pool_choice)

        _, self.dist_head = create_classifier(self.num_features, self.num_classes, pool_type=self.global_pool_choice)
...
def bossnet_T0(pretrained=False, **kwargs):
    ...
    """
    model = BossNet(encoding=[[1], [1], [1, 1, 1, 0, 1], [0, 0, 0, 1, 0, 0, 0, 0, 0]], avg_down=True, act_layer=SiLU, #act_layer=nn.SiLU,
                    attn_layer='se')
...
@register_model
def bossnet_T0_mix(pretrained=False, **kwargs):
    """Constructs a ResNet-26-D model.
    stem_width=32, stem_type='deep'
    """
    model = BossNet(encoding=[[1], [1], [1, 1, 1, 0, 1], [0, 0, 0, 1, 0, 0, 0, 0, 0]], avg_down=True, act_layer=SiLU, #act_layer=nn.SiLU,
...
@register_model
def bossnet_T0_mix_v2_Q(pretrained=False, **kwargs):
    """Constructs a ResNet-26-D model.
    stem_width=32, stem_type='deep'
    """
    model = BossNet(encoding=[[1], [1], [3, 2, 1, 0, 3], [0, 0, 0, 1, 0, 0, 0, 0, 0]], avg_down=True, act_layer=SiLU, #act_layer=nn.SiLU,
                    attn_layer='se')
    model.default_cfg = _cfg()
    return model
...
@register_model
def bossnet_T0_mix_v3_Q(pretrained=False, **kwargs):
    """Constructs a ResNet-26-D model.
    stem_width=32, stem_type='deep'
    """
    model = BossNet(encoding=[[1, 1], [1, 1, 1, 1], [3, 1, 0, 2, 2], [0, 0, 1, 0, 0]], avg_down=True, act_layer=SiLU, #act_layer=nn.SiLU,
                    attn_layer='se')
    model.default_cfg = _cfg()
    return model

@register_model
def bossnet_T1_mix_v3_Q(pretrained=False, **kwargs):
    """Constructs a ResNet-26-D model.
    stem_width=32, stem_type='deep'
    """
    model = BossNet(encoding=[[1, 1], [1, 1, 1, 1], [3, 1, 0, 2, 2], [0, 0, 1, 0, 0]], avg_down=True, act_layer=SiLU, #act_layer=nn.SiLU,
...
# Distilled models
@register_model
def bossnet_T0_mix_v3_distilled_Q(pretrained=False, **kwargs):
    """Constructs a ResNet-26-D model.
    stem_width=32, stem_type='deep'
    """
    model = DistilledBossNet(encoding=[[1, 1], [1, 1, 1, 1], [3, 1, 0, 2, 2], [0, 0, 1, 0, 0]], avg_down=True, act_layer=SiLU, #act_layer=nn.SiLU,
                    attn_layer='se')
    model.default_cfg = _cfg()
    return model

@register_model
def bossnet_T1_mix_v3_distilled_Q(pretrained=False, **kwargs):
    """Constructs a ResNet-26-D model.
    stem_width=32, stem_type='deep'
    """
    model = DistilledBossNet(encoding=[[1, 1], [1, 1, 1, 1], [3, 1, 0, 2, 2], [0, 0, 1, 0, 0]], avg_down=True, act_layer=SiLU, #act_layer=nn.SiLU,
...
@register_model
def bossnet_T0_mix_v4_distilled_Q(pretrained=False, **kwargs):
    """Constructs a ResNet-26-D model.
    stem_width=32, stem_type='deep'
    """
    model = DistilledBossNet(encoding=[[1, 1], [1, 3, 1, 1], [3, 1, 0, 3, 3], [0, 0, 1, 0, 0]], avg_down=True, act_layer=SiLU, #act_layer=nn.SiLU,
                    attn_layer='se')
    model.default_cfg = _cfg()
    return model

@register_model
def bossnet_T1_mix_v4_distilled_Q(pretrained=False, **kwargs):
    """Constructs a ResNet-26-D model.
    stem_width=32, stem_type='deep'
    """
    model = DistilledBossNet(encoding=[[1, 1], [1, 3, 1, 1], [3, 1, 0, 3, 3], [0, 0, 1, 0, 0]], avg_down=True, act_layer=SiLU, #act_layer=nn.SiLU,
