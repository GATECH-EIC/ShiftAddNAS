...
def main(args):
    ...
    print(f"Top-1 accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.2f}%")
    print(f"Top-5 accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc5']:.2f}%")
    return
...
def evaluate(data_loader, model, device):
    ...
    for images, target in metric_logger.log_every(data_loader, 50, header):
        ...
        with torch.no_grad():
            ...
            output = model(images)
            end_time = time.time()
            print('inference one image cost: {}s'.format(end_time - start_time))
