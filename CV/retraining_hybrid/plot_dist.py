...
from boss_candidates.adder.adder import Adder2D
from boss_candidates.deepshift.modules import Conv2dShift
...
def main(args):
    ...
    for m in model.modules():
        if isinstance(m, Adder2D):
            weight = m.adder.data.cpu().reshape(-1)
    ...
    sub_fig_count = 0
    for m in model.modules():
        if isinstance(m, Conv2dShift):
            weight = m.shift.data.cpu().reshape(-1)
            weight = weight[weight.nonzero()]
            print(layer_count, '-th layer: ', weight.shape)
            layer_count += 1
            # plot
            if sub_fig_count < 4:
                sub_fig_count += 1
                plot_fig_weights(weight.reshape(1, -1), sub_fig_count)
    print('total shift layers: ', layer_count)
    plt.tight_layout()
    plt.savefig(args.output_dir+'/shift.pdf')
    ...
    for m in model.modules():
        if isinstance(m, nn.Conv2d):
            weight = m.weight.data.cpu().reshape(-1)
            weight = weight[weight.nonzero()]
            weight = weight[weight < 0.5 * weight.max()]
            weight = weight[weight > 0.5 * weight.min()]
            print(layer_count, '-th layer: ', weight.shape)
            layer_count += 1
            # plot
            if layer_count >= 60 and layer_count < 64:
                sub_fig_count += 1
                plot_fig_weights(weight.reshape(1, -1), sub_fig_count)
