...
class ResConv_ws_dist(nn.Module):
    ...
    def forward(self, x):
        ...
        weight_collection = [
            self.shared_weight['downsample_d.0'].reshape(-1),
            self.shared_weight['conv1_d'].reshape(-1),
            self.shared_weight['downsample.0'].reshape(-1),
            self.shared_weight['conv1'].reshape(-1),
            self.shared_weight['conv2'].reshape(-1),
            self.shared_weight['conv3'].reshape(-1)
            ]
...
class ResAdd_ws_dist(nn.Module):
    ...
    def forward(self, x):
        ...
        if fmap_size > self.fmap_size:
            ...
            weight_shape = self.shared_weight['downsample_d.0'].shape
            weight = self.shared_weight['downsample_d.0'].reshape(-1)
            ...
            if not isinstance(self.downsample, nn.Identity):
                ...
                weight_shape = self.shared_weight['downsample.0'].shape
                weight = self.shared_weight['downsample.0'].reshape(-1)
                weight_min = weight.min()
                weight_max = weight.max()
                for i in range(self.stage):
        ...
        adder_collection = [new_downsample_d_weight, new_conv1_d_weight, new_downsample_weight, new_conv1_weight, new_conv2_weight, new_conv3_weight]
