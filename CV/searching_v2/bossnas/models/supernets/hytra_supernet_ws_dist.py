...
class shared_parameters(nn.Module):
    def __init__(self, inplanes, planes, fmap_size, up_inplanes, stride=1, cardinality=1, base_width=64, reduce_first=1, dilation=1, expansion=4):
        ...
        # self.fuckkkk = nn.Parameter(torch.randn(outplanes, 3, 3, 3))

        # # self.shared_weight = {}
        # self.shared_weight = nn.ParameterDict({})
        # if up_inplanes is not None:
        #     # self.shared_weight['downsample_d'] = nn.Parameter(torch.randn(outplanes, up_inplanes, 3, 3)).cuda()
        #     self.shared_weight.update({'downsample_d': nn.Parameter(torch.randn(outplanes, up_inplanes, 3, 3))})
        # if inplanes != outplanes:
        #     # self.shared_weight['downsample'] = nn.Parameter(torch.randn(outplanes, inplanes, 1, 1)).cuda()
        #     self.shared_weight.update({'downsample': nn.Parameter(torch.randn(outplanes, inplanes, 1, 1))})
        # if up_inplanes is not None:
        #     # self.shared_weight['conv1_d'] = nn.Parameter(torch.randn(first_planes, up_inplanes, 1, 1)).cuda()
        #     self.shared_weight.update({'conv1_d': nn.Parameter(torch.randn(first_planes, up_inplanes, 1, 1))})
        ...
        # self.shared_weight.update({'conv1': nn.Parameter(torch.randn(first_planes, inplanes, 1, 1))})
        # self.shared_weight.update({'conv2': nn.Parameter(torch.randn(width, first_planes // cardinality, 3, 3))})
        # self.shared_weight.update({'conv3': nn.Parameter(torch.randn(outplanes, width, 1, 1))})

        if up_inplanes is not None:
            self.downsample_d = nn.Parameter(torch.randn(outplanes, up_inplanes, 3, 3))
        if inplanes != outplanes:
            self.downsample = nn.Parameter(torch.randn(outplanes, inplanes, 1, 1))
        if up_inplanes is not None:
            self.conv1_d = nn.Parameter(torch.randn(first_planes, up_inplanes, 1, 1))

        self.conv1 = nn.Parameter(torch.randn(first_planes, inplanes, 1, 1))
        self.conv2 = nn.Parameter(torch.randn(width, first_planes // cardinality, 3, 3))
        self.conv3 = nn.Parameter(torch.randn(outplanes, width, 1, 1))
...
class MixOps(nn.Module):
    def __init__(self, inc, outc, stage, to_dispatch=False, init_op_index=None, ):
        ...
        for depth in range(stage_depth):
            ...
            # self.whatever = nn.Conv2d(123, 45, kernel_size=3)

            self.depth_parameters = shared_parameters(inc, outc // 4, fmap_size, up_inc)
            self.ws.append(self.depth_parameters)
            ...
            for prim in PRIMITIVES:
                if prim.startswith('ResAtt'):
                    ...
                    conv = OPS[prim](inc, outc // 4, fmap_size, up_inc)
                    self._mix_ops.append(conv)
                elif prim == 'ResAdd_ws_dist':
                    # add = OPS[prim](inc, outc // 4, fmap_size, up_inc, getattr(self, 'depth_{}_paramters'.format(depth)).shared_weight)
                    add = OPS[prim](inc, outc // 4, fmap_size, up_inc, self.ws[depth])
                    self._mix_ops.append(add)
                elif prim == 'ResAdd':
                    add = OPS[prim](inc, outc // 4, fmap_size, up_inc)
                    self._mix_ops.append(add)
                elif prim == 'ResShift_ws_dist':
                    # shift = OPS[prim](inc, outc // 4, fmap_size, up_inc, getattr(self, 'depth_{}_paramters'.format(depth)).shared_weight)
                    shift = OPS[prim](inc, outc // 4, fmap_size, up_inc, self.ws[depth])
    ...
    def forward(self, x, forward_index, plot=False):
        ...
        return self._mix_ops[forward_index](x, plot=plot)

    def get_params(self, forward_index):
        params = 0
        ...
        if self.inc == 64:
            ...
            outc = self._mix_ops[forward_index].inc
            downsample_cfg = str(inc) + '_' + str(outc)
            params += sum(p.numel() for p in self.downsamples[downsample_cfg].parameters() if p.requires_grad)

        params += sum(p.numel() for p in self._mix_ops[forward_index].parameters() if p.requires_grad)

        if self._mix_ops[forward_index].shared_weight is not None:
            params = sum(p.numel() for p in self._mix_ops[forward_index].shared_weight.parameters() if p.requires_grad)

        return params

    def del_redundancy(self):
        self._mix_ops[forward_index].del_redundancy()
...
class Block(nn.Module):
    ...
    def forward(self, x, forwad_list=None, plot=False):
        assert len(forwad_list) == len(self._block_layers)
        kl_loss = 0
        for i, layer in enumerate(self._block_layers):
            x, _kl_loss = layer(x, forward_index=forwad_list[i], plot=plot)
            kl_loss += _kl_loss
        return x, kl_loss

    def del_redundancy(self):
        for i, layer in enumerate(self._block_layers):
            layer.del_redundancy(forward_index=forwad_list[i])

    def reset_params(self):
        self.apply(reset)

    def get_params(self, forward_list):
        assert len(forward_list) == len(self._block_layers)
        params = 0
        for i, layer in enumerate(self._block_layers):
            params += layer.get_params(forward_index=forward_list[i])

        return params


@BACKBONES.register_module
class SupernetHyTra_ws_dist(nn.Module):

    def __init__(self, to_dispatch=False, init_op_list=None, block_layers_num=None, num_classes=100):
        super(SupernetHyTra_ws_dist, self).__init__()
    ...
    def forward_feature(self, x, start_block, forward_op=None, block_op=True, plot=False):
        # outs = []
        if start_block == 0:
            x = self._stem(x)
        ...
        for i, block in enumerate(self._blocks):
            if i < start_block:  # if start_block = 1 , forward will skip the block 0 and 1
                continue
            if block_op:
                x, _kl_loss = block(x, forward_op, plot=plot)
                kl_loss += _kl_loss
                # print('after block op: ', x.shape)
            else:
                x, _kl_loss = block(x, forward_op[sum(self._op_layers_list[:i]):sum(
                    self._op_layers_list[:(i + 1)])], plot=plot)
                kl_loss += _kl_loss
    ...
    def forward(self, x, plot=False, config=None):
        # example
        ...
        # random select one
        if config is None:
            self.forward_op = random.sample(self.all_configs, 1)[0]
        else:
            self.forward_op = config

        # print(forward_op)

        x, kl_loss = self.forward_feature(x, start_block=0, forward_op=self.forward_op, block_op=False, plot=plot)
        x = self.global_pool(x)
        x = self.fc(x)
        return x, kl_loss

    def get_params(self, cand):
        # x = self._stem(x)
        stem_params = sum(p.numel() for p in self._stem.parameters() if p.requires_grad)

        module_params = 0
        for i, block in enumerate(self._blocks):
             module_params += block.get_params(cand[sum(self._op_layers_list[:i]):sum(self._op_layers_list[:(i + 1)])])

        pool_params = sum(p.numel() for p in self.global_pool.parameters() if p.requires_grad)
        fc_params = sum(p.numel() for p in self.fc.parameters() if p.requires_grad)

        total_params = stem_params + module_params + pool_params + fc_params

        # print('stem: ', stem_params, 'module: ', module_params, 'pool: ', pool_params, 'fc: ', fc_params)
    ...
    def del_redundancy(self):
        for i, block in enumerate(self._blocks):
            ...
            x, _kl_loss = block.del_redundancy(forward_op[sum(self._op_layers_list[:i]):sum(
...
@register_model
def Supernet_v3(pretrained=False, **kwargs):
    model = SupernetHyTra_ws_dist()
