...
from bossnas.models.operations.adder.quantize import quantize, quantize_grad, QuantMeasure, calculate_qparams
...
class Conv2dShift(_ConvNdShift):
    ...
    def forward(self, input):
        self.shift.data = ste.clamp(self.shift.data, *self.shift_range)
        # print("self.shift.data",self.shift.data)
        shift_rounded = ste.round(self.shift, self.rounding)
        # print("shift_rounded",shift_rounded)
        ...
        if self.quant_bits > 0:
            ...
            input_fixed_point = ste.round_fixed_point(input, quant_bits=self.quant_bits)
        if self.bias is not None:
            bias_fixed_point = ste.round_fixed_point(self.bias, quant_bits=self.quant_bits)
        else:
            bias_fixed_point = None
...
class _ConvNdShift_ws(nn.Module):

    __constants__ = ['stride', 'padding', 'dilation', 'groups', 'bias', 'padding_mode']

    def __init__(self, shared_weight, kernel_size, stride,
                 padding, dilation, transposed, output_padding,
                 groups, bias, padding_mode,
                 check_grad=False, freeze_sign=False,
                 rounding='deterministic', weight_bits=5):
        super(_ConvNdShift_ws, self).__init__()
        ...
        if transposed:
            # self.shift = nn.Parameter(tensor_constructor(
            #     in_channels, out_channels // groups, *kernel_size))
            self.shift = self.shared_weight['weight'].reshape(self.shared_weight['weight_shape'])
            # self.sign = nn.Parameter(tensor_constructor(
            #     in_channels, out_channels // groups, *kernel_size),
            #     requires_grad = (freeze_sign == False))
            self.sign = self.shared_weight['weight'].reshape(self.shared_weight['weight_shape'])
        else:
            # self.shift = nn.Parameter(tensor_constructor(
            #     out_channels, in_channels // groups, *kernel_size))
            self.shift = self.shared_weight['weight'].reshape(self.shared_weight['weight_shape'])
...
class Conv2dShift_ws(_ConvNdShift_ws):
    def __init__(self, shared_weight, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1,
                 bias=True, padding_mode='zeros',
                 check_grad=False, freeze_sign=False, use_kernel=False, use_cuda=True, rounding='deterministic', weight_bits=5, threshold=0.3, quant_bits=16):
        kernel_size = _pair(kernel_size)
        ...
        self.threshold = threshold
        self.quant_bits = quant_bits
        super(Conv2dShift_ws, self).__init__(
            shared_weight, kernel_size, stride, padding, dilation,
            False, _pair(0), groups, bias, padding_mode,
            check_grad, freeze_sign, rounding, weight_bits)
    ...
    def forward(self, input):
        ...
        if self.threshold is None:
            ...
            sign_rounded_signed = ste.sign(ste.myround(self.sign, self.threshold))
        weight_ps = ste.unsym_grad_mul(2**shift_rounded, sign_rounded_signed)
        print("weight_ps:",weight_ps.type)
        # input_fixed_point = ste.round_fixed_point(input, quant_bits=self.quant_bits)
        if self.quant_bits > 0:
            input_fixed_point = ste.round_fixed_point(input, quant_bits=self.quant_bits)
        else:
