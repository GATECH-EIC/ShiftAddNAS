...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        outputs, kl_loss = model(samples, plot=False)
        # print(kl_loss)
        loss = criterion(outputs, targets) + 10 * kl_loss
...
@torch.no_grad()
def evaluate(data_loader, model, device, config=None):
    ...
    for images, target in metric_logger.log_every(data_loader, 50, header):
        ...
        # with torch.cuda.amp.autocast():
        with torch.no_grad():
            output, kl_loss = model(images, plot=True, config=config)
