...
class PEG_Shift(nn.Module):
    ...
    def __init__(self, dim, stride):
        ...
        # self.conv = nn.Conv2d(dim, dim, kernel_size=3, stride=stride, padding=1, groups=dim)
        self.conv = modules_q.Conv2dShiftQ(dim, dim, kernel_size=3, stride=stride, padding=1, groups=dim, bias=False, weight_bits=5, quant_bits=16)
...
class ResShift_ws_dist(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, cardinality=1, base_width=64, reduce_first=1, dilation=1,
        ...
        if up_inplanes is not None:
            self.downsample_d = nn.Sequential(
                # nn.Conv2d(up_inplanes, outplanes, 3, stride=2, padding=1, bias=False),
                modules_q.Conv2dShiftQ(up_inplanes, outplanes, kernel_size=3, stride=2, padding=1, bias=False, weight_bits=5, quant_bits=16),
                nn.BatchNorm2d(outplanes),
                act_layer(inplace=True))
        ...
        if inplanes != outplanes:
            self.downsample = nn.Sequential(
                # nn.Conv2d(inplanes, outplanes, 1, stride=1, padding=0, bias=False),
                modules_q.Conv2dShiftQ(inplanes, outplanes, kernel_size=1, stride=1, padding=0, bias=False, weight_bits=5, quant_bits=16),
                nn.BatchNorm2d(outplanes),
                act_layer(inplace=True))
        ...
        if up_inplanes is not None:
            # self.conv1_d = nn.Conv2d(up_inplanes, first_planes, kernel_size=1, bias=False)
            self.conv1_d = modules_q.Conv2dShiftQ(up_inplanes, first_planes, kernel_size=1, bias=False, weight_bits=5, quant_bits=16)
            ...
            self.bn1_d = norm_layer(first_planes)
        # self.conv1 = nn.Conv2d(inplanes, first_planes, kernel_size=1, bias=False)
        self.conv1 = modules_q.Conv2dShiftQ(inplanes, first_planes, kernel_size=1, bias=False, weight_bits=5, quant_bits=16)
        ...
        # self.conv3 = nn.Conv2d(width, outplanes, kernel_size=1, bias=False)
        self.conv3 = modules_q.Conv2dShiftQ(width, outplanes, kernel_size=1, bias=False, weight_bits=5, quant_bits=16)
    ...
    def forward(self, x, plot=False):
        ...
        if fmap_size > self.fmap_size:
            ...
            weight_max = weight.max()
            for i in range(self.stage):
                temp_shift = weight * self.downsample_d_shift_affine[i]
            ...
            self.downsample_d[0].weight = new_downsample_d_shift.reshape(weight_shape)
            ...
            weight_max = weight.max()
            for i in range(self.stage):
                temp_shift = weight * self.conv1_d_shift_affine[i]
            ...
            self.conv1_d.weight = conv1_d_shift.reshape(weight_shape)
            ...
            if not isinstance(self.downsample, nn.Identity):
                ...
                weight_max = weight.max()
                for i in range(self.stage):
                    temp_shift = weight * self.downsample_shift_affine[i]
                ...
                self.downsample[0].weight = downsample_shift.reshape(weight_shape)
            ...
            weight_max = weight.max()
            for i in range(self.stage):
                temp_shift = weight * self.conv1_shift_affine[i]
            ...
            self.conv1.weight = conv1_shift.reshape(weight_shape)
        ...
        weight_max = weight.max()
        for i in range(self.stage):
            temp_shift = weight * self.conv2_shift_affine[i]
        ...
        self.conv2.weight = conv2_shift.reshape(weight_shape)
        ...
        weight_max = weight.max()
        for i in range(self.stage):
            temp_shift = weight * self.conv3_shift_affine[i]
        ...
        self.conv3.weight = conv3_shift.reshape(weight_shape)
