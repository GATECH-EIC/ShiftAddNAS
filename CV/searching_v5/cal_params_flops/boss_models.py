...
from cal_params_flops.boss_candidates.bot_op import ResAtt
from cal_params_flops.boss_candidates.resnet_op import ResConv, ResAdd, ResShift
...
@register_model
def customized_model(pretrained=False, encoding=None, **kwargs):
    """Constructs a ResNet-26-D model.
    stem_width=32, stem_type='deep'
    """
    if encoding == None:
        ...
        print(encoding)
        model = BossNet(encoding=encoding, avg_down=True, act_layer=SiLU, #act_layer=nn.SiLU,
                    attn_layer='se')
    model.default_cfg = _cfg()
    return model
