...
path_cifar_kl = '../searching_v2/work_dirs/cifar100/Supernet_v2/arch_ranking_resume.pth.tar'
path_cifar_ws = '../searching_v5/work_dirs/cifar100/Supernet_v5/arch_ranking.pth.tar'
# path_cifar_ws = '/mnt/scratch/v_youhaoran/BossNAS/searching_hy_v5/work_dirs/cifar100/Supernet_v5_prev/arch_ranking.pth.tar'

info_cifar = torch.load(path_cifar_kl)
...
index = np.argsort(cifar_test_acc_1)
index = index < 200

print('Score w.r.t. 200 archs')
print('---'*10)
print('Ranking correlation in terms of top-1 training accuracy: ')
print('kendalltau: ', stats.kendalltau(np.array(cifar_train_acc_1)[index], np.array(cifar_ws_train_acc_1)[index]))
print('pearsonr: ', stats.pearsonr(np.array(cifar_train_acc_1)[index], np.array(cifar_ws_train_acc_1)[index]))
print('spearmanr: ', stats.spearmanr(np.array(cifar_train_acc_1)[index], np.array(cifar_ws_train_acc_1)[index]))
print('---'*10)
print('Ranking correlation in terms of top-1 testing accuracy: ')
print('kendalltau: ', stats.kendalltau(np.array(cifar_test_acc_1)[index], np.array(cifar_ws_test_acc_1)[index]))
print('pearsonr: ', stats.pearsonr(np.array(cifar_test_acc_1)[index], np.array(cifar_ws_test_acc_1)[index]))
print('spearmanr: ', stats.spearmanr(np.array(cifar_test_acc_1)[index], np.array(cifar_ws_test_acc_1)[index]))
print('---'*10)
...
plt.tight_layout()
plt.savefig('./work_dirs/corr_v2_v5.pdf')
plt.savefig('./work_dirs/corr_v2_v5.svg')
