...
@register_model('transformersuper_shiftadd')
class TransformerSuperModel_shiftadd(SuperFairseqEncoderDecoderModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--encoder-block-types', nargs='+', default=['lightweight_conv', 'lightweight_add', 'lightweight_shiftadd',
                                    'self_attention', 'self_attention+lightweight_conv', 'self_attention+lightweight_add', 'self_attention+lightweight_shiftadd'])
        parser.add_argument('--decoder-block-types', nargs='+', default=['lightweight_conv', 'lightweight_add', 'lightweight_shiftadd',
                                    'self_attention', 'self_attention+lightweight_conv', 'self_attention+lightweight_add', 'self_attention+lightweight_shiftadd'])
    ...
    def build_model(cls, args, task):
        ...
        return TransformerSuperModel_shiftadd(encoder, decoder)

    @classmethod
    def build_encoder(cls, args, src_dict, embed_tokens):
        return TransformerEncoder_shiftadd(args, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return TransformerDecoder_shiftadd(args, tgt_dict, embed_tokens)


class TransformerEncoder_shiftadd(FairseqEncoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.layers.extend([
            TransformerEncoderLayer_shiftadd(args, layer_idx=i, kernel_size=self.encoder_kernel_size_list[i])
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i < self.sample_layer_num:
                layer.set_sample_config(is_identity_layer=False,
                                        ...
                                        block_types=self.block_types[i][0])
...
class TransformerDecoder_shiftadd(FairseqIncrementalDecoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.layers = nn.ModuleList([])
        self.layers.extend([
            TransformerDecoderLayer_shiftadd(args, layer_idx=i, no_encoder_attn=no_encoder_attn, kernel_size=self.decoder_kernel_size_list[i])
            for i in range(self.super_layer_num)
        ])
...
class TransformerEncoderLayer_shiftadd(nn.Module):
    """Encoder layer block.
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, layer_idx, kernel_size=0):
        super().__init__()
        ...
        # self_attention
        assert args.block_types[3] == 'self_attention'
        ...
        assert args.block_types[4] == 'self_attention+lightweight_conv'
        ...
        assert args.block_types[5] == 'self_attention+lightweight_add'
        ...
        assert args.block_types[6] == 'self_attention+lightweight_shiftadd'
        ...
        if self.my_mode == 'train':
            ...
            for i in range(self.act_path):
                ...
                if activate < 3:
                    ...
                    res.append(self.super_block[activate](x))
                else:
                    # self_attn or multi-branch
                    res.append(self.super_block[activate](query=x, key=x, value=x, key_padding_mask=encoder_padding_mask)[0])
            # print(res[0].shape)
            # print(res[1].shape)
            x = res[0]
            ...
            if activate < 3:
                x = self.super_block[activate](x)
            else:
                x = self.super_block[activate](query=x, key=x, value=x, key_padding_mask=encoder_padding_mask)[0]
        else:
...
class TransformerDecoderLayer_shiftadd(nn.Module):
    """Decoder layer block.
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, layer_idx, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False, kernel_size=0):
        super().__init__()
...
@register_model_architecture('transformersuper_shiftadd', 'transformersuper_shiftadd')
...
@register_model_architecture('transformersuper_shiftadd', 'transformersuper_shiftadd_iwslt_de_en')
...
@register_model_architecture('transformersuper_shiftadd', 'transformersuper_shiftadd_wmt_en_de')
def transformer_wmt_en_de(args):
    base_architecture(args)

@register_model_architecture('transformersuper_shiftadd', 'transformersuper_shiftadd_wmt_en_fr')
def transformer_wmt_en_de(args):
    base_architecture(args)

# parameters used in the "Attention Is All You Need" paper (Vaswani et al., 2017)
@register_model_architecture('transformersuper_shiftadd', 'transformersuper_shiftadd_vaswani_wmt_en_de_big')
...
@register_model_architecture('transformersuper_shiftadd', 'transformersuper_shiftadd_vaswani_wmt_en_fr_big')
def transformer_vaswani_wmt_en_fr_big(args):
    args.dropout = getattr(args, 'dropout', 0.1)
    transformer_vaswani_wmt_en_de_big(args)


@register_model_architecture('transformersuper_shiftadd', 'transformersuper_shiftadd_wmt_en_de_big')
...
@register_model_architecture('transformersuper_shiftadd', 'transformersuper_shiftadd_wmt_en_de_big_t2t')
