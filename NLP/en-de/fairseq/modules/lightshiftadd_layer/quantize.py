...
class UniformQuantize(InplaceFunction):
    ...
    def forward(ctx, input, num_bits=None, qparams=None, flatten_dims=_DEFAULT_FLATTEN,
        ...
        min_scale = torch.tensor(1e-8, dtype=scale.dtype).expand_as(scale).cuda()
...
class my_QLinear(nn.Module):
    """docstring for my_QLinear"""
    def __init__(self):
        super(my_QLinear, self).__init__()
        self.quantize_input = QuantMeasure(shape_measure=(1, 1), flatten_dims=(1, -1), momentum=0.1)

    def forward(self, input, weight, bias, num_bits, num_bits_grad):
        if num_bits == -1:
            qinput = input
            qweight = weight
            qbias = bias
            ...
            if bias is not None:
                qbias = quantize(bias, num_bits=num_bits, flatten_dims=(0, -1))
            else:
                qbias = None
...
def my_QLinear_F(input, weight, bias, num_bits=8, num_bits_grad=8):
    if num_bits == -1:
        qinput = input
        qweight = weight
        qbias = bias
    else:
        quantize_input = QuantMeasure(shape_measure=(1, 1), flatten_dims=(1, -1), momentum=0.1)
        qinput = quantize_input(input, num_bits)
        weight_qparams = calculate_qparams(weight, num_bits=num_bits, flatten_dims=(1, -1), reduce_dim=None)
        qweight = quantize(weight, qparams=weight_qparams)
        if bias is not None:
            qbias = quantize(bias, num_bits=num_bits, flatten_dims=(0, -1))
