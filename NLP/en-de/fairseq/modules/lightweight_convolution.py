...
def LightweightAdd(input_size, kernel_size=1, padding_l=None, num_heads=1,
                    weight_dropout=0., weight_softmax=False, bias=False, with_linear=False, out_dim=None):
    if torch.cuda.is_available():
        try:
            from fairseq.modules.lightadd_layer import LightaddLayer
            print('use add!')
            return LightaddLayer(input_size, kernel_size=kernel_size,
                                  padding_l=padding_l, num_heads=num_heads,
                                  weight_dropout=weight_dropout,
                                  weight_softmax=weight_softmax, bias=bias,
                                  with_linear=with_linear, out_dim=out_dim)
...
def LightweightShiftadd(input_size, kernel_size=1, padding_l=None, num_heads=1,
                    weight_dropout=0., weight_softmax=False, bias=False, with_linear=False, out_dim=None):
    if torch.cuda.is_available():
        try:
            from fairseq.modules.lightshiftadd_layer import LightshiftaddLayer
            print('use shiftadd!')
            return LightshiftaddLayer(input_size, kernel_size=kernel_size,
                                  padding_l=padding_l, num_heads=num_heads,
                                  weight_dropout=weight_dropout,
                                  weight_softmax=weight_softmax, bias=bias,
                                  with_linear=with_linear, out_dim=out_dim)
