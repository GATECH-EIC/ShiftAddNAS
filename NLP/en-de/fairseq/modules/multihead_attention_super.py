...
from .quantize import my_QLinear_F, my_QLinear
...
class MultiheadAttentionSuper(nn.Module):
    ...
    def __init__(self, super_embed_dim, num_heads, is_encoder, super_kdim=None, super_vdim=None, dropout=0., bias=True,
        ...
        self.my_in_proj = my_QLinear()
        self.my_in_proj_q = my_QLinear()
        self.my_in_proj_k = my_QLinear()
        self.my_in_proj_v = my_QLinear()
        self.my_in_proj_qkv = my_QLinear()
    ...
    def forward(self, query, key, value, key_padding_mask=None, incremental_state=None,
                need_weights=True, static_kv=False, attn_mask=None, num_bits=-1, num_bits_grad=-1):
        ...
        # print('num of head: ', self.num_heads)
        ...
        if self.self_attention:
            # self-attention
            q, k, v = self.in_proj_qkv(query, num_bits=num_bits, num_bits_grad=num_bits_grad)
        elif self.encoder_decoder_attention:
            # encoder-decoder attention
            q = self.in_proj_q(query, num_bits=num_bits, num_bits_grad=num_bits_grad)
            if key is None:
                assert value is None
                k = v = None
            else:
                k = self.in_proj_k(key, num_bits=num_bits, num_bits_grad=num_bits_grad)
                v = self.in_proj_v(key, num_bits=num_bits, num_bits_grad=num_bits_grad)

        else:
            q = self.in_proj_q(query, num_bits=num_bits, num_bits_grad=num_bits_grad)
            k = self.in_proj_k(key, num_bits=num_bits, num_bits_grad=num_bits_grad)
            v = self.in_proj_v(value, num_bits=num_bits, num_bits_grad=num_bits_grad)
        ...
        # print('attn weight: ', attn_weights.shape)
        ...
        attn = self.out_proj(attn, num_bits=num_bits, num_bits_grad=num_bits_grad)
    ...
    def in_proj_qkv(self, query, num_bits=-1, num_bits_grad=-1):
        return self._in_proj(query, sample_dim=self.sample_q_embed_dim, num_bits=num_bits, num_bits_grad=num_bits_grad).chunk(3, dim=-1)

    def in_proj_q(self, query, num_bits=-1, num_bits_grad=-1):
        if self.qkv_same_dim:
            return self._in_proj(query, end=self.qkv_dim, sample_dim=self.sample_q_embed_dim, num_bits=num_bits, num_bits_grad=num_bits_grad)
        else:
            bias = self.in_proj_bias
            if bias is not None:
                bias = bias[:self.qkv_dim]
            # return F.linear(query, self.q_proj_weight[..., :self.sample_q_embed_dim], bias)
            # return my_QLinear_F(query, self.q_proj_weight[..., :self.sample_q_embed_dim], bias, num_bits=num_bits, num_bits_grad=num_bits_grad)
            return self.my_in_proj_q(query, self.q_proj_weight[..., :self.sample_q_embed_dim], bias, num_bits=num_bits, num_bits_grad=num_bits_grad)

    def in_proj_k(self, key, num_bits=-1, num_bits_grad=-1):
        if self.qkv_same_dim:
            return self._in_proj(key, start=self.qkv_dim, end=2 * self.qkv_dim, sample_dim=self.sample_kv_embed_dim, num_bits=num_bits, num_bits_grad=num_bits_grad)
        else:
            ...
            # return my_QLinear_F(key, weight[..., :self.sample_kv_embed_dim], bias, num_bits=num_bits, num_bits_grad=num_bits_grad)
            return self.my_in_proj_k(key, weight[..., :self.sample_kv_embed_dim], bias, num_bits=num_bits, num_bits_grad=num_bits_grad)

    def in_proj_v(self, value, num_bits=-1, num_bits_grad=-1):
        if self.qkv_same_dim:
            return self._in_proj(value, start=2 * self.qkv_dim, sample_dim=self.sample_kv_embed_dim, num_bits=num_bits, num_bits_grad=num_bits_grad)
        else:
            ...
            # return my_QLinear_F(value, weight[..., :self.sample_kv_embed_dim], bias, num_bits=num_bits, num_bits_grad=num_bits_grad)
            return self.my_in_proj_v(value, weight[..., :self.sample_kv_embed_dim], bias, num_bits=num_bits, num_bits_grad=num_bits_grad)

    def _in_proj(self, input, sample_dim, start=0, end=None, num_bits=-1, num_bits_grad=-1):
        ...
        # return my_QLinear_F(input, weight, bias, num_bits=num_bits, num_bits_grad=num_bits_grad)
        return self.my_in_proj(input, weight, bias, num_bits=num_bits, num_bits_grad=num_bits_grad)
