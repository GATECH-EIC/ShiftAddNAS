...
from deepshift.quantize import quantize, quantize_grad, QuantMeasure, calculate_qparams
...
class LinearShift_super(nn.Module):
    def __init__(self, in_features, out_features, bias=True, check_grad=False, freeze_sign=False, use_kernel=False, use_cuda=True, rounding='deterministic', weight_bits=5, threshold=None):
        super(LinearShift_super, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.use_kernel = use_kernel
        self.check_grad = check_grad
    ...
    def _sample_parameters(self):
        self.samples['weight'] = sample_weight(self.shift, self.sample_in_dim, self.sample_out_dim)
        self.samples['sign'] = sample_weight(self.sign, self.sample_in_dim, self.sample_out_dim)
    ...
    def forward(self, input, num_bits=-1, num_bits_grad=-1):

        # print(self.profiling)

        if not self.profiling:
            """ normal one """
            self.samples['weight'].data = ste.clamp(self.samples['weight'].data, *self.shift_range)
            shift_rounded = ste.round(self.samples['weight'], rounding=self.rounding)
            if self.threshold == None:
                sign_rounded_signed = ste.sign(ste.round(self.samples['sign'], rounding=self.rounding))
            else:
                sign_rounded_signed = ste.sign(round(self.samples['sign'], self.threshold))
            weight_ps = ste.unsym_grad_mul(2**shift_rounded, sign_rounded_signed)

            if self.use_kernel:
                return LinearShiftFunction.apply(input, self.samples['weight'], self.samples['sign'], self.bias, self.conc_weight, self.use_kernel, self.use_cuda, self.rounding, self.shift_range)
            else:
                return torch.nn.functional.linear(input, weight_ps, self.samples['bias'])
        else:
            """ if profiling flops """
            self._sample_parameters()
            return torch.nn.functional.linear(input, self.samples['weight'], self.samples['bias'])
...
class Conv2dShift(_ConvNdShift):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 ...
                 check_grad=False, freeze_sign=False, use_kernel=False, use_cuda=True, rounding='deterministic', weight_bits=5, threshold=0.3, quant_bits=16):
