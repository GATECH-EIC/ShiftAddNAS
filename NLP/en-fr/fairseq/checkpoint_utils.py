...
def save_checkpoint(args, trainer, epoch_itr, val_loss, name=''):
    ...
    # print(epoch, end_of_epoch, updates)
    end_of_epoch = True
    # print(epoch, end_of_epoch)

    checkpoint_conds = collections.OrderedDict()
    checkpoint_conds[name] = False if name == '' else True
    ...
    print(checkpoint_conds, len(checkpoints))
    ...
    # test the BLEUs
    if 'wmt' in args.arch and not 'en_fr' in args.arch:
        command = 'bash configs/wmt14.en-de/test.sh' + \
                  ' {}/checkpoint_last.pt'.format(args.save_dir) + \
                  ' configs/wmt14.en-de/subtransformer/{}.yml'.format(args.save_dir.split('/')[-1]) + \
                  ' normal 7 test'
        os.system(command)
    elif 'wmt' in args.arch and 'en_fr' in args.arch:
        command = 'bash configs/wmt14.en-fr/test.sh' + \
                  ' {}/checkpoint_last.pt'.format(args.save_dir) + \
                  ' configs/wmt14.en-fr/subtransformer/{}.yml'.format(args.save_dir.split('/')[-1]) + \
                  ' normal 7 test'
        os.system(command)
    else:
        print('no related test func.')
        # exit()
...
def load_checkpoint(args, trainer):
    ...
    if extra_state is not None and 'train_iterator' in extra_state and not args.reset_dataloader:
