    ...
    main_block = """
                if (padding_l == {pad}) {{
                    AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.scalar_type(), "lightadd_forward", ([&] {{
                        lightadd_forward_kernel<{k}, {b_size}, {pad}, scalar_t>
...
"""
    ...
    with open("lightadd_cuda_forward.cu", 'w') as forward:
        forward.write(head)
        for seq in seqs:
    ...
    main_block = """
                if (padding_l == {p}) {{
                    AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.scalar_type(), "lightadd_backward", ([&] {{
                        lightadd_grad_wrt_input_kernel<{k}, {b_size}, {p}, scalar_t>
...
"""
    ...
    weight_grad_short = """
                        ...
                        lightadd_grad_wrt_weights_firstpass_short_kernel<{k}, {b_size}, {p}, scalar_t>
                        ...
                        lightadd_grad_wrt_weights_secondpass_short_kernel<{k}, {b_size}, scalar_t>
...
"""
    ...
    weight_grad = """
                        ...
                        lightadd_grad_wrt_weights_firstpass_kernel<{k}, {b_size}, {p}, scalar_t>
                        <<<gradBlocks, {b_size}, 0, stream>>>(
                                input.data<scalar_t>(),
                                gradOutput.data<scalar_t>(),
                                minibatch,
                        ...
                        lightadd_grad_wrt_weights_secondpass_kernel<{k}, {b_size}, scalar_t>
                        <<<weightGradSecondpassBlocks, {b_size}, 0, stream>>>(
                                tempSumGradFilters.data<float>(),
                                minibatch,
                                numFiltersInBlock,
...
"""
    ...
    with open("lightadd_cuda_backward.cu", 'w') as backward:
        backward.write(head)
