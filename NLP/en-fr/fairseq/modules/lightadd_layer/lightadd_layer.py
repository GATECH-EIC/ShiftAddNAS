...
class lightaddFunction(Function):

    @staticmethod
    def forward(ctx, x, weights, padding_l):
        ctx.padding_l = padding_l
        # print('lightadd: ', x.shape, weights.shape, padding_l)
        outputs = lightadd_cuda.forward(x, weights, padding_l)
        variables = [x, weights]
    ...
    def backward(ctx, grad_output):
        outputs = lightadd_cuda.backward(
...
class LightaddLayer(nn.Module):
    def __init__(
            ...
            with_linear=False,
            out_dim=None):
        super(LightaddLayer, self).__init__()
    ...
    def forward(self, x, incremental_state=None, num_bits=-1, num_bits_grad=-1):
        ...
        # during training time, use CUDA kernel
            ...
            # output = lightaddFunction.apply(x, weight, self.padding_l).permute(2, 0, 1)
            ...
            if num_bits > 0:
                ...
                output = lightaddFunction.apply(qx, qweight, self.padding_l).permute(2, 0, 1)
            else:
                output = lightaddFunction.apply(x, weight, self.padding_l).permute(2, 0, 1)
