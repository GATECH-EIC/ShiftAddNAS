...
#include "lightshiftadd_cuda_forward.cu"
#include "lightshiftadd_cuda_backward.cu"
...
__global__
void lightshiftadd_forward_kernel(const scalar_t* input,
...
template<int FS, int SB, int padding_l, typename scalar_t>
__global__
void lightshiftadd_grad_wrt_input_kernel(
    const scalar_t* input,
    const scalar_t* filters,
    const scalar_t* ori_input, // new added by HR; May. 5, 2021
    int minibatch,
    int sequenceLength,
    int numFeatures,
  ...
  // new
  const scalar_t* ori_input_feature = &ori_input[IOOffset];
  ...
  __shared__ scalar_t temp[SB + FS];
  // new
  __shared__ scalar_t temp_in[SB + FS];

  const int padding = FS - padding_l - 1;
  zeroSharedMem<FS, SB, padding>(temp);
  // new
  zeroSharedMem<FS, SB, padding>(temp_in);
  ...
  for (int i = 0; i < numIterations; ++i) {
    ...
    // new
    load_input_to_shared<FS, SB, padding>(ori_input_feature, inputOffset, sequenceLength,
                                 i, numIterations, false, temp_in);
    ...
    for (int j = 0; j < FS; ++j) {
      // out += filter[j] * temp[tid + j];
      // scalar_t ht = HARDTANH(filter[j] - ori_input[IOOffset + tid + outputOffset]);
      // scalar_t ht = filter[j] - ori_input[IOOffset + tid + outputOffset];
      if ((outputOffset + tid) < sequenceLength) {
        scalar_t ht = filter[j] - ori_input_feature[tid + outputOffset];
        // out += temp[tid + j] * ht;
        if (ht < -1){
          out -= temp[tid + j];
        } else if (ht > 1){
        ...
        scalar_t ht = filter[j] - temp_in[tid + j];
        if (ht < -1){
          out -= temp[tid + j];
        } else if (ht > 1){
          out += temp[tid + j];
...
template<int FS, int SB, int padding_l, typename scalar_t>
__global__
void lightshiftadd_grad_wrt_weights_firstpass_short_kernel(
    const scalar_t* input,
    const scalar_t* gradInput,
    const scalar_t* ori_weight, // added by HR, May. 5, 2021
    int minibatch,
    int sequenceLength,
    int numFeatures,
    int numFiltersInBlock,
  ...
  for (int idxInFilterBlock = 0; idxInFilterBlock < numFiltersInBlock; ++idxInFilterBlock) {
    ...
    for (int i = 0; i < numIterations; ++i) {
      ...
      for (int j = 0; j < FS; j++) {
        ...
        accumWeights[j] += (tempInput[inputIndex] - ori_weight[filterIdx * FS + j]) * tempGrad;
      }
...
__global__
void lightshiftadd_grad_wrt_weights_secondpass_short_kernel(
    const float* input,
    const int minibatch,
...
template<int FS, int SB, int padding_l, typename scalar_t>
__global__
void lightshiftadd_grad_wrt_weights_firstpass_kernel(
    const scalar_t* input,
    const scalar_t* gradInput,
    int minibatch,
    int sequenceLength,
...
template<int FS, int SB, typename scalar_t>
__global__
void lightshiftadd_grad_wrt_weights_secondpass_kernel(
    const float* input,
    const int minibatch,
    const int numFiltersInBlock,
    scalar_t* output) {
