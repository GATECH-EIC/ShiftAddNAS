...
def main_encoder(config, save_name='example.csv', num_bits=32):
    ...
    for i in range(config["encoder_layers"]):
        ...
        elif block_type == "lightweight_conv":
            OPs_list = []
            idx = 0

            kernel_size = kernel_size_list[i]
            padding = (kernel_size - 1) // 2
            ops = lightweight(embedding_dim=embedding_dim, num_head=num_head, idx=idx, num_token=num_token, kernel_size=kernel_size, padding=padding)
            idx += len(ops)
            OPs_list.extend(ops)
...
def main_decoder(config, save_name='example.csv', num_bits=32):
    ...
    for i in range(config["decoder_layers"]):
        ...
        if block_type == "self_attention":
            ...
            kernel_size = kernel_size_list[i]
            padding = (kernel_size - 1) // 2
            ops = lightweight(embedding_dim=embedding_dim, num_head=num_head_1, idx=idx, num_token=num_token, kernel_size=kernel_size, padding=padding)
            idx += len(ops)
            OPs_list.extend(ops)
...
if __name__ == '__main__':
    ...
    #     "encoder_ffn_dim": [3072, 1024, 1024, 3072, 1024, 1024],
    #     "encoder_layers": 6,
    #     "encoder_num_head": [8, 8, 4, 8, 8, 8],
    #     "encoder_block_types": ["lightweight_conv", "lightweight_conv", "lightweight_conv", "lightweight_conv", "lightweight_conv", "lightweight_conv"],
    #     # decoder
    #     "decoder_embedding_dim": 768,
    #     "decoder_ffn_dim": [3072, 3072, 3072, 3072, 3072, 3072],
    #     "decoder_layers": 6,
    #     "decoder_num_head_1": [4, 8, 8, 8, 8, 8],
    #     "decoder_num_head_2": [4, 4, 8, 8, 8, 8],
    #     "decoder_block_types": ["self_attention+lightweight_conv", "self_attention+lightweight_conv",
    #         "self_attention+lightweight_conv", "self_attention+lightweight_conv", "self_attention+lightweight_conv",
    #         "self_attention+lightweight_conv"],
    #     # ende
    #     "arbitrary_ende": [2, 1, -1, 2, 1, -1]
    # }

    # num_bits = args.num_bits

    # if os.path.exists('conv_200ms_encoder_32bits.csv'):
    #     os.remove('conv_200ms_encoder_32bits.csv')

    # if os.path.exists('conv_200ms_decoder_32bits.csv'):
    #     os.remove('conv_200ms_decoder_32bits.csv')

    # main_encoder(config, save_name='conv_200ms_encoder_32bits.csv')
    # main_decoder(config, save_name='conv_200ms_decoder_32bits.csv')
    ...
    config = {
        # encoder
        "encoder_embedding_dim": 512,
        "encoder_ffn_dim": [2048, 3072, 3072, 2048, 1024, 1024],
        "encoder_layers": 6,
        "encoder_num_head": [8, 4, 8, 4, 4, 4],
        "encoder_block_types": ["lightweight_conv", "lightweight_conv", "lightweight_conv", "lightweight_conv", "lightweight_conv", "lightweight_conv"],
        # decoder
        "decoder_embedding_dim": 768,
        "decoder_ffn_dim": [3072, 3072, 3072],
        "decoder_layers": 3,
        "decoder_num_head_1": [8, 8, 8],
        "decoder_num_head_2": [4, 8, 8],
        "decoder_block_types": ["self_attention+lightweight_conv", "self_attention+lightweight_conv",
            "self_attention+lightweight_conv", "self_attention+lightweight_conv", "self_attention+lightweight_conv",
            "self_attention+lightweight_conv"],
        # ende
        "arbitrary_ende": [2, 1, -1]
    }

    num_bits = args.num_bits

    if os.path.exists('conv_150ms_encoder_32bits.csv'):
        os.remove('conv_150ms_encoder_32bits.csv')

    if os.path.exists('conv_150ms_decoder_32bits.csv'):
        os.remove('conv_150ms_decoder_32bits.csv')

    main_encoder(config, save_name='conv_150ms_encoder_32bits.csv')
    main_decoder(config, save_name='conv_150ms_decoder_32bits.csv')
